...
Pour que les listes soient lues correctement par l'interface, vos fichiers distants `payloads.json` et `pkg.json` doivent respecter l'une des structures ci-dessous.
...
[
  {
    ...
    "url": "[https://depot.com/payloads/kstuff.elf](https://depot.com/payloads/kstuff.elf)"
  },
  {
    "name": "FTP Server",
    "version": "1.0",
    ...
    "url": "[https://depot.com/payloads/ftpserver.elf](https://depot.com/payloads/ftpserver.elf)"
  }
]
...
1. Ouvrez `index.html` (vue globale) ou `index.1.html` (vue avec onglets) dans votre navigateur.
2. Renseignez l'adresse IP de votre console PS5 dans le champ **Configuration**.
3. Choisissez une source dans la liste déroulante ou sélectionnez `-- Ajouter une URL personnalisée --`.
4. Sélectionnez votre payload ELF ou votre package PKG.
5. Cliquez sur **Injecter l'ELF** ou **Installer le PKG**.
